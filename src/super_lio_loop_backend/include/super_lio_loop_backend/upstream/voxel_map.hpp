...
#include "super_lio_loop_backend/upstream/tools.hpp"
#include "super_lio_loop_backend/upstream/preintegration.hpp"
#include <thread>
...
inline vector<double> plane_eigen_value_thre;
...
public:
  ...
  void damping_iter(vector<IMUST> &x_stats, LidarFactor &voxhess, deque<IMU_PRE*> &imus_factor, Eigen::MatrixXd* hess)
  {
    ...
    for(int i=0; i<3; i++)
    {
      ...
      double tl1 = wall_time_seconds();
      residual2 = only_residual(x_stats_temp, voxhess, imus_factor);
      double tl2 = wall_time_seconds();
      // printf("onlyresi: %lf\n", tl2-tl1);
      resitime += tl2 - tl1;
